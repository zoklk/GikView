...
- **결과**:
  ...
  ![fio sync write latency by percentile](assets/testA_microSD_fio-latency.png) <!-- microSD vs USB 3.0, fio 30초 4K sync write, p50~p99.99 log scale -->
...
- **결과**:
  ...
  ![Cilium 3-Pod CPU sum during empty Service churn](assets/testB_cilium_cpu.png) <!-- baseline 60s + load 120s, 3 Pod CPU 합계 시계열 -->
...
- **결과**:
  ...
  ![Node CPU (usr+sys) with/without Argo CD](assets/testC_node_cpu.png) <!-- 좌: argocd 운영 중(baseline, 120s), 우: scale=0(120s), 노드 CPU usr+sys -->
