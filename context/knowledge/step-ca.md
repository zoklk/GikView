...
정책은 provisioner 단위로 선언. 본 프로젝트는 device CN 패턴 (`device-{MAC 뒷 6자리}`) 만 허용하도록 정규식 정책 적용.

```yaml
# X5C provisioner 안에 추가
- type: "X5C"
  name: "device-bootstrap"
  roots: "<base64>"
  policy:
...
화이트리스트 CN 목록은 Mapping Generator CronJob 이 `device-room-mapping` ConfigMap 에서 자동 생성하여 step-ca 의 policy 섹션을 갱신. policy 변경 시 step-ca rollout restart 필요 (Reloader 가 트리거).
...
- **Policy 변경 시 rollout restart 필수**: ca.json 의 policy 또는 provisioner 변경은 hot reload 안 됨. ConfigMap 갱신 후 Reloader 가 deployment rollout 트리거. rollout 중 약 5~10 초 발급 거부 윈도우 발생.
