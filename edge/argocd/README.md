...
| `4` | visibility | web-metrics-exporter | DynamoDB `gikview-metrics` poll → `web_connect_total` 재노출. prometheus 스크랩 타겟(`:9102`), IAM Roles Anywhere read-only |
| `4` | visibility | prometheus | node-exporter + 기존 타겟(emqx/influxdb/cert-manager/telegraf/edge-gateway/web-metrics-exporter) 스크랩. etcd 노출 선행 |
...
argocd app sync cert-manager reloader step-issuer mapping-generator \
                ...
                node-exporter web-metrics-exporter prometheus alertmanager telegraf-freshness grafana cloudflared
```
