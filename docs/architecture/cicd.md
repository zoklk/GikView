...
- **선택**: 정적 검증 + 런타임 검증 + 이미지 빌드(linux/arm64) + GHCR Push를 로컬 하네스가 수행. GitHub Actions는 lint/format 체크만
- **대안**: GitHub Actions가 전체 CI 담당 (멀티아키텍처 빌드 포함)
...
- **트레이드오프**: 로컬 PC 환경 의존. PC 꺼져 있으면 빌드 불가. 환경 표준화 책임은 개발자 본인
