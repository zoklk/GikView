...
| `gikview-ws-handler-{stage}-role` | `gikview-connections-{stage}` CRUD · `gikview-rooms-{stage}` Scan · `gikview-metrics-{stage}` UpdateItem · API GW ManageConnections · CloudWatch Logs | `gikview-ws-handler-{stage}` |
...
| `gikview-web-visibility` | step-ca Intermediate CA (IAM Roles Anywhere) | DynamoDB GetItem → `gikview-metrics-dev` · `gikview-metrics-prod` |

edge gateway 가 mTLS 인증서로 STS credentials 교환 후 rooms 테이블 업데이트. Streams 가 이 변경을 broadcast Lambda 로 전파.

`gikview-web-visibility` 는 edge 의 web-metrics-exporter 가 사용하는 **read-only** role. Trust Anchor 는 `gikview-edgegw` 와 동일 step-ca Intermediate. dev/prod 클러스터가 **단일 role 재사용** → 인라인 정책이 `gikview-metrics-dev`·`gikview-metrics-prod` 양쪽 GetItem 허용. exporter 는 polling read 만 하므로 write 권한 없음.
...
웹 demand 계측용 단조 카운터. handler 가 `$connect` 마다 누적, edge 의 web-metrics-exporter 가 IAM Roles Anywhere read role 로 polling 해 Prometheus `web_connect_total` 로 재노출.

```yaml
KeySchema:
  - AttributeName: metric
    KeyType: HASH        # String, 카운터 종류 식별자 (현재 "connect" 1종)
AttributeDefinitions:
  - AttributeName: metric
    AttributeType: S
# TTL/Streams 없음 — 단순 카운터
```

- `metric` (PK, String): 카운터 키. 현재 `connect` 만 사용.
- `n` (Number): 단조 증가 카운터. handler 가 `ADD n :d` 로 누적 — 매 connect 동기 write 아님(연결 크리티컬 패스 보호). warm 컨테이너 메모리에 누적 후 30s 또는 50건마다 1회 flush, best-effort(flush 실패는 connect 응답 안 막음).
- exporter 는 `GetItem(ConsistentRead=false)` 로 polling — eventually-consistent(half RCU). item 부재(아직 connect 없음)는 값 0 으로 정상 처리.
...
| handler   | `METRICS_TABLE`     | `gikview-metrics-prod`     | `gikview-metrics-dev` |
...
▎ handler METRICS_TABLE 는 옵션 — 미설정 시 계측 no-op(코드 선배포 안전). 설정 시 위 표대로 stage별 테이블.
...
| `METRICS_TABLE` (handler) | `gikview-metrics-dev` | `gikview-metrics-prod` |
