...
## **gikview-ws-authorizer-{stage} Lambda 내부에서 직접 검증** (GIST IdP JWKS endpoint 호출)

클라이언트가 JWT를 헤더로 전달 불가 (WebSocket 프로토콜 제약) → query string으로 전달.
...
입력: event.queryStringParameters.token (JWT access_token)
처리:

token 없음 → "Unauthorized" 예외 (API GW 401, 연결 거부)
GIST IdP JWKS endpoint 공개키 조회
(https://api.account.gistory.me/.well-known/openid-configuration → jwks_uri)
JWT 검증: 서명, exp, iss, aud
성공 → IAM Allow 정책 + context 반환
실패 → "Unauthorized" 예외
...
주의: JWKS 공개키는 Lambda 실행 컨텍스트(전역 변수)에 캐싱. 매 요청마다 IdP 호출 방지.
```
...
─── $connect ───
  ...
  3. return HTTP 200 (필수, 누락 시 API GW 연결 거부)
