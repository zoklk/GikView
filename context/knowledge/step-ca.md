...
제거하거나 `roots`/`policy` 를 비우고 redeploy. `device-renewal`(roots=intermediate_ca)·`admin`(JWK)
만 남음. `ca.json` 변경이라 rollout — `step-ca-whitelist` Reloader 와 동일하게 약 5~10 초 발급 거부
윈도우.
...
- 부트스트랩 신원의 CN(`smoke-bootstrap`)은 화이트리스트에 없어도 된다 — policy 는 요청된 device
  CN(#10(a) 가 `step-ca-whitelist` 첫 항목을 뽑아 씀)만 검사.
