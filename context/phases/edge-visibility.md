...
- **healthchecks.io check**: check 1개 생성(period 1m, grace 5–10m) → 알림 채널 연결 → ping URL 발급 → 환경별 `alertmanager-healthchecks` Secret 등록. 무료티어로 충분. ping URL 토큰성, git 평문 금지.
...
- **외부 watchdog (결정 9)**: `Watchdog` alert(상시 발화) 전용 라우트를 `healthchecks` webhook receiver 로 — `group_wait 0s`/`group_interval 1m`/`repeat_interval 1m`, `send_resolved: false`. ping URL = `alertmanager-healthchecks` Secret(`url_file`). healthchecks.io grace(5–10m) 초과 시 외부 통보 → 알림 파이프 전체 장애를 클러스터 밖에서 감지. 기존 Discord 24h watchdog 라우트는 self-referential 이라 제거.
