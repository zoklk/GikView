...
step-certificates:
  ...
  # ── ca.json 머지 (결정 c: device CN 화이트리스트 → X5C cert-template templateData.allowedCNs; + B: admin JWK encryptedKey) — 전부 step-certificates 1.30.x native 훅 ──
  extraVolumes:
    ...
    - name: merge-ca-config
      ...
      args:
        - |
          ...
          # 결정 c — step-ca-whitelist 가 있으면 각 X5C provisioner 의 cert-template templateData.allowedCNs 를 화이트리스트로 덮어씀.
...
### Policy — device CN 화이트리스트 (결정 c: X5C cert-template + `{{ fail }}` 가드)

**OSS(자체 호스팅) step-ca 는 policy 를 authority 레벨에서만 지원한다.** provisioner-단위 policy 블록은
hosted Smallstep Certificate Manager 전용 — `ca.json` 의 `provisioners[].policy` 에 써도 step-ca 가 조용히
무시한다 (provisioner config 구조체에 policy 필드 없음; `authority/provisioner/options.go` 의
`X509Options.AllowedNames`/`DeniedNames` 는 `json:"-"`). authority-레벨 policy 는 모든 provisioner(admin
포함)에 적용돼 부적합 — admin(JWK)은 step-issuer 가 K8s 워크로드 cert 발급에 쓰므로 device CN 제약을 받으면
안 됨.
...
→ 모든 device cert 거부(**fail-closed**; 실제로는 mapping-generator 가 sync-wave -1 에 항상 먼저 생성).
step-ca 는 `template`/`templateData` 를 hot-reload 못 하므로 `step-ca-whitelist` 변경 시 Reloader
...
> 변경 후 step-ca 재적용은 ArgoCD app refresh 또는 수동 rollout 으로.
...
### Bootstrap CA 회전 / 부트스트랩 경로 폐쇄 (사람, 필요 시 — docs ADR 13번)

CRL/OCSP 미도입(docs ADR 7번)이라 개별 부트스트랩 인증서는 폐기 불가 → 폐기·회전은 **Bootstrap CA
...
- **EMQX 클라이언트 검증 trust anchor = Root (Intermediate 아님)**: 부트스트랩 인증서 격리는 EMQX TLS 핸드셰이크가 아니라 ACL `no_match = deny` 가 담당 — EMQX 5.8.6(Erlang/OTP)이 `ssl_options.partial_chain` 을 노출 안 해 Intermediate 를 단독 trust anchor 로 못 씀. 디바이스/워크로드는 핸드셰이크에 `leaf + Intermediate` 제시 필수. 상세: docs ADR 12번, `context/knowledge/emqx.md`.

- **BearSSL 클라이언트 IP SAN 미지원**: ESP8266 BearSSL 클라이언트는 서버 인증서의 IP SAN 매칭을 안 함 (DNS SAN 만 지원). 본 phase 는 step-ca 를 NodePort 로 외부 노출(security.md 결정 11)하지만, IP SAN 을 박아도 디바이스가 검증 안 하므로 step-ca 서버 인증서 `dnsNames` 는 DNS-only(노드 IP 미포함)로 둔다. 디바이스는 노드 IP 로 접속하되 IP SAN 검증은 생략 — 검증 환경은 `WiFiClientSecure.setInsecure()` 로 우회, 운영 trust 전략(펌웨어에 하드코딩한 DNS 명을 노드 IP 로 매핑(또는 학내 DNS) + 그 명을 `dnsNames` 에 추가)은 별도 ADR.
