...
# ── Processor 1: occupancy(int) → occupied(bool) 변환 ────────────────────────
# 디바이스(sensor.cpp) 가 occupancy 를 int 0/1 로 publish. InfluxDB field 는 bool 로 통일.
[[processors.converter]]
  order = 1
  [processors.converter.fields]
    boolean = ["occupancy"]
  [processors.converter.rename]
    occupancy = "occupied"

# ── Processor 2: device_id → room_id 변환 ────────────────────────────────────
[[processors.lookup]]
  order = 2
  # telegraf-lookup ConfigMap (mapping-generator 생성): device_id → room_id CSV
  files         = ["/lookup/mapping.csv"]
  # CSV 헤더: device_id,room_id
  key           = "device_id"
  fields        = ["room_id"]
...
MQTT `inputs.mqtt_consumer` 가 topic 을 tag 로 자동 포함. `processors.regex` (order 0) 가 topic 에서 device_id 추출 → `processors.converter` (order 1) 가 occupancy(int) → occupied(bool) 변환 + rename → `processors.lookup` (order 2) 가 `telegraf-lookup` CSV 에서 room_id 매핑.
...
- **occupancy → occupied 변환 위치**: sensor.cpp(end phase 완료) 가 JSON 필드명 `occupancy` int 로 publish — 펌웨어 재배포 비용 회피 위해 Telegraf 단에서 `processors.converter` + rename 으로 처리. Edge Gateway 도 동일 변환 (DynamoDB attribute `occupied` bool).
