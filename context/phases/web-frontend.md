...
- **인증 (OIDC PKCE)** `services/auth.ts`:
  ...
  - `scope`: `email name student_id offline_access` (openid 미요청 → id_token 안 받음. 서버가 access_token 으로 신원 검증하므로 불요)
  - `userStore`: `localStorage` (access+refresh_token 보관 → 새로고침 세션 생존)
  ...
  - `signinRedirect({ prompt: 'consent' })` (openid 없음 → nonce 불요, offline_access → consent 필요)
